...
export const FeedbackForm: React.FC = () => {
  ...
  return (
    <div className={cn("min-h-screen", dark ? "bg-[#0A1128]" : "bg-[#EEF2F8]")} style={{ fontFamily: "'Inter', sans-serif" }}>
      {/* Header */}
      <header className={cn("sticky top-0 z-20 backdrop-blur-sm border-b px-6 py-3.5 flex items-center justify-between", headerBg)}>
        <div className="flex items-center gap-3">
          <button onClick={() => navigate("/student")} className={cn("p-2 rounded-xl border transition-colors cursor-pointer", dark ? "bg-white/5 border-white/10 text-[#5A6E8E] hover:text-white" : "border-[#0B3D91]/10 bg-white text-[#5A6E8E] hover:text-[#0B3D91]")}>
            <ChevronLeft size={16} />
          </button>
          <div>
            <div className="flex items-center gap-2 text-sm">
              <span className={textSub}>Student Dashboard</span>
              <ChevronRight size={14} className={textSub} />
              <span className={cn("font-semibold", textPrimary)}>Feedback Form</span>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        {/* Faculty Card */}
        <div className={cn("rounded-2xl p-6 border shadow-sm", cardBg)}>
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#3B82F6] to-[#0B3D91] flex items-center justify-center shrink-0">
              <Users size={22} className="text-white" strokeWidth={1.5} />
            </div>
            <div className="flex-1">
              <div className="flex items-center justify-between">
                ...
                <div className={cn("text-xs font-semibold flex items-center gap-1", textPrimary)}>
                  Progress: {progressPercent}%
                </div>
              </div>
              <h2 className={cn("font-bold text-lg leading-tight", textPrimary)} style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{subject?.name}</h2>
              <p className={cn("text-sm mt-0.5", textSub)}>{faculty?.name} &nbsp;·&nbsp; {user?.branch || "Computer Science"}</p>
              
              <div className={cn("h-1.5 w-full rounded-full mt-4", dark ? "bg-white/10" : "bg-[#EEF2F8]")}>
        ...
        <div className="space-y-4">
          {questions.length === 0 ? (
            ...
            <div className={cn("rounded-3xl p-8 border shadow-sm transition-all duration-300", cardBg)}>
               <div className="flex items-center justify-between mb-6">
                 ...
                 <span className={cn("text-xs font-semibold", textSub)}>
                   {currentQ?.category}
                 </span>
               </div>
               
               <p className={cn("text-xl font-medium mb-8 leading-relaxed", textPrimary)} style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                 {currentQ?.text}
               </p>
          ...
          <div className="flex items-center justify-between pt-4">
            <button
              ...
              className={cn("flex items-center gap-2 px-5 py-3 rounded-xl font-semibold text-sm transition-all cursor-pointer border-0", 
            ...
            {currentIdx < questions.length - 1 ? (
              <button
                ...
                className={cn("flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all shadow-md cursor-pointer border-0", 
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 text-white font-bold text-sm hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-600/25 disabled:opacity-50 disabled:pointer-events-none cursor-pointer border-0"
              >
